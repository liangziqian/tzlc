...
export default {
    ...
    mounted: function() {
        ...
        let values = sessionStorage.getItem("userid");
        if(values){
          this.jsessionid = values
        }else{
          this.jsessionid = $this.$route.query.userId
        }
        //是否缓存了用户id
        this.$nextTick(function() {
            // 中奖名单
            ...
            //判断是否要登陆
           // let value = sessionStorage.getItem("userid"); //有数值代表二维码过来，没有则是app过来的
            if($this.$route.query.userId){//app
            	  sessionStorage.setItem('isLogin', true)
                  sessionStorage.setItem('userid', $this.$route.query.userId)
                
             }else{
            	  if($this.jsessionid==null||$this.jsessionid==undefined){
                    Toast('请先登录')
                    $this.$router.push({
		                path: '/login',
		                query: {
